...
export const GameLoop = (entities, { touches, events, dispatch }) => {
    ...
    if (head.nextMove === 0) {
        ...
        if (head.position[0] + head.xspeed < 0 || head.position[0] + head.xspeed > Constants.GRID_SIZE - 1 ||
            head.position[1] + head.yspeed < 0 || head.position[1] + head.yspeed > Constants.GRID_SIZE - 1) {
